...
def str_to_nparray(str):
    return np.frombuffer(str.encode(), dtype=np.uint8)
...
def get_aug_config(scale_factor=0.25, rot_factor=30, rot_prob=0.6, color_factor=0.2):
    scale = np.clip(np.random.randn(), 0, 1.0) * scale_factor + 1.0
...
def augmentation_list(img_list, bbox, data_split, input_img_shape, scale_factor=0.25, rot_factor=30, rot_prob=0.6, color_factor=0., do_flip=False):
    if data_split == 'train':
        scale, rot, color_scale = get_aug_config(scale_factor, rot_factor, rot_prob, color_factor)
    else:
        scale, rot, color_scale = 1.0, 0.0, np.array([1, 1, 1])
    ...
    for idx, img in enumerate(img_list):
        img, trans, inv_trans = generate_patch_image(img, bbox, scale, rot, do_flip, input_img_shape)
        # NOTE: only add color jitting on RGB image
        aug_img_list.append(img)

    return aug_img_list, trans, inv_trans, rot, scale
