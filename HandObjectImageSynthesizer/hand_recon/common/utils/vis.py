...
from hand_recon.common.utils.preprocessing import generate_patch_image
...
def render_badcase_mesh(ori_img, gen_img, bbox, pred_ori_mesh, pred_gen_mesh, gt_mesh, face, cam_param, do_flip, out_shape=(128, 128)):
    # mesh
    ...
    pred_ori_rgb, _, _ = generate_patch_image(pred_ori_rgb, bbox, scale=1, rot=0, do_flip=do_flip, out_shape=out_shape)
    valid_mask = (pred_ori_depth > 0)[:, :, None].astype(np.uint8)
    valid_mask, _, _ = generate_patch_image(valid_mask, bbox, scale=1, rot=0, do_flip=do_flip, out_shape=out_shape, warp_flag=cv2.INTER_NEAREST)
    valid_mask = valid_mask[:, :, None]
    pred_ori_img = pred_ori_rgb * valid_mask + ori_img * (1 - valid_mask)

    # pred gen image
    pred_gen_rgb = pred_gen_rgb[:, :, :3].astype(np.float32)
    pred_gen_rgb, _, _ = generate_patch_image(pred_gen_rgb, bbox, scale=1, rot=0, do_flip=do_flip, out_shape=out_shape)
    ...
    gt_rgb, _, _ = generate_patch_image(gt_rgb, bbox, scale=1, rot=0, do_flip=do_flip, out_shape=out_shape)
...
def render_rot_mesh(ori_img, bbox, rot_mesh, gt_mesh, face, cam_param, do_flip):
    # mesh
    rot_mesh = trimesh.Trimesh(rot_mesh, face)
    gt_mesh = trimesh.Trimesh(gt_mesh, face)

    rot = trimesh.transformations.rotation_matrix(np.radians(180), [1, 0, 0])
    rot_mesh.apply_transform(rot)
    gt_mesh.apply_transform(rot)

    material = pyrender.MetallicRoughnessMaterial(metallicFactor=0.0, alphaMode='OPAQUE', baseColorFactor=(1.0, 1.0, 0.9, 1.0))
    rot_mesh = pyrender.Mesh.from_trimesh(rot_mesh, material=material, smooth=True)
    gt_mesh = pyrender.Mesh.from_trimesh(gt_mesh, material=material, smooth=True)
    ...
    # render rot results
    rot_mesh_node = scene.add(rot_mesh, 'rot_mesh')
    rot_rgb, rot_depth = renderer.render(scene, flags=pyrender.RenderFlags.RGBA)
    scene.remove_node(rot_mesh_node)
    ...
    # rot image
    rot_rgb = rot_rgb[:, :, :3].astype(np.float32)
    rot_rgb, _, _ = generate_patch_image(rot_rgb, bbox, scale=1, rot=0, do_flip=do_flip, out_shape=(128, 128))
    valid_mask = (rot_depth > 0)[:, :, None].astype(np.uint8)
    ...
    valid_mask = valid_mask[:, :, None]
    rot_rgb = rot_rgb * valid_mask + ori_img * (1 - valid_mask)
    ...
    img_list = [ori_img, gt_img, ori_img, rot_rgb]
    for i in range(len(img_list)):
        img_list[i] = cv2.copyMakeBorder(np.copy(img_list[i]), 3, 3, 3, 3, cv2.BORDER_CONSTANT, value=[0, 0, 0])
...
def render_for_vis(ori_img, gen_img, bbox, pred_ori_mesh, pred_gen_mesh, gt_mesh, face, cam_param, do_flip, out_shape=(128, 128)):
    # mesh
    pred_ori_mesh = trimesh.Trimesh(pred_ori_mesh, face)
    pred_gen_mesh = trimesh.Trimesh(pred_gen_mesh, face)
    gt_mesh = trimesh.Trimesh(gt_mesh, face)
