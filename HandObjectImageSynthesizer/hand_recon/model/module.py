...
class Pose2Feat(nn.Module):
    ...
    def forward(self, img_feat, joint_heatmap_3d):
        joint_heatmap_3d = joint_heatmap_3d.view(-1, self.joint_num * self.cfg.data.output_hm_shape[0], self.cfg.data.output_hm_shape[1],
                                                 self.cfg.data.output_hm_shape[2])
...
class ParamLightPredWeights(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.backbone = ResNetBackbone(self.cfg)
        # self.fc = make_linear_layers([2048, 3072, 512], use_bn=True)
        self.lobe = torch.load('/research/d4/rshr/xuhao/code/ARHandLightDP/common/utils/sphere_locations/sphere_locations_N{}.pth'.format(
            self.cfg.model.num_lights)).cuda().float()
...
class OriDenseNet(nn.Module):
    ...
    def __init__(self,
        ...
        for i, num_layers in enumerate(block_config):
            Dense_block = _DenseBlock(num_layers=num_layers,
