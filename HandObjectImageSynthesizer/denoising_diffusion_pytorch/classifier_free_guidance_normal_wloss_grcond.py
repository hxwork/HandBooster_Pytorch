...
class ResnetBlock(nn.Module):
    ...
    def __init__(self, dim, dim_out, *, time_emb_dim=None, classes_emb_dim=None, groups=8):
        super().__init__()
        self.mlp = nn.Sequential(nn.SiLU(), nn.Linear(int(time_emb_dim) + int(classes_emb_dim), dim_out * 2)) if exists(time_emb_dim) or exists(classes_emb_dim) else None
...
class Unet(nn.Module):
    ...
    def __init__(
        ...
        self.classes_emb = nn.Sequential(nn.Linear(4, dim), nn.GELU())
...
class GaussianDiffusion(nn.Module):
    ...
    def model_predictions(self, x, t, classes, condition, cond_scale=3., clip_x_start=False):
        model_output = self.model.forward_with_cond_scale(x, t, classes, condition, cond_scale=cond_scale)
    ...
    def p_mean_variance(self, x, t, classes, condition, cond_scale, clip_denoised=True):
        preds = self.model_predictions(x, t, classes, condition, cond_scale)
    ...
    @torch.no_grad()
    def p_sample(self, x, t: int, classes, condition, cond_scale=3., clip_denoised=True):
    ...
    @torch.no_grad()
    def p_sample_loop(self, input, cond_scale=3., return_cond=True, return_all_timesteps=False):
        device = self.betas.device
        rgb = input['img']
        condition = input['condition']
        normal = input['normal']
        img = torch.randn(rgb.size(), device=device)
        imgs = [img]
        classes = p3dt.matrix_to_quaternion(p3dt.axis_angle_to_matrix(input['mano_pose'][:, :3]))

        condition = torch.cat((condition, normal), dim=1)
        condition = normalize_to_neg_one_to_one(condition)

        x_start = None

        for t in tqdm(reversed(range(0, self.num_timesteps)), desc='sampling loop time step', total=self.num_timesteps):
            img, x_start = self.p_sample(img, t, classes, condition, cond_scale)
            if t % 50 == 0:
                imgs.append(img)
        ...
        if return_cond:
            if return_all_timesteps:
                return img, condition, rgb, imgs
            else:
                return img, condition, rgb
        else:
            if return_all_timesteps:
                return img, imgs
            else:
    ...
    @torch.no_grad()
    def ddim_sample(self, input, cond_scale=3., clip_denoised=True, return_cond=True, return_all_timesteps=False):
        device = self.betas.device
        rgb = input['img']
        condition = input['condition']
        normal = input['normal']
    ...
    @torch.no_grad()
    def sample(self, input, cond_scale=3., return_cond=True, return_all_timesteps=False):
        sample_fn = self.p_sample_loop if not self.is_ddim_sampling else self.ddim_sample
        return sample_fn(input, cond_scale=cond_scale, return_cond=return_cond, return_all_timesteps=return_all_timesteps)
    ...
    def p_losses(self, x_start, t, *, classes, condition, noise=None):
        ...
        mask = torch.sum(condition[:, :3, :, :], dim=1, keepdim=True) > 30
        weight = torch.where(mask, torch.ones_like(mask), torch.ones_like(mask) * 0.1)
        # print(weight.size(), loss.size())
        loss = weight * loss
    ...
    def forward(self, input, *args, **kwargs):
        ...
        condition = torch.cat((condition, normal), dim=1)
        condition = normalize_to_neg_one_to_one(condition)
        return self.p_losses(img, t, classes=classes, condition=condition, *args, **kwargs)
