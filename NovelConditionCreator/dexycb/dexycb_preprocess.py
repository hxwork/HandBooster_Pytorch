...
from preprocessing import load_img, get_bbox, process_bbox, cam2pixel

import pyrender
from NovelConditionCreator.dexycb.vis import vis_keypoints_with_skeleton, render_hand_obj_property, render_hand_obj_condition
...
def load_data():
    ...
    if args.debug:
        db = COCO(osp.join(annot_path, 'DEX_YCB_{}_24_data.json'.format(args.data_split)))
    else:
        db = COCO(osp.join(annot_path, 'DEX_YCB_{}_data.json'.format(args.data_split)))
...
def main(anns, imgs):
    # NOTE train: 406888, test: 78768, train with bbox: 401507
    all_obj = load_ycb_objects()

    if args.data_split == 's0_train':
        real_info_path = '/dexycb/dexycb_s0/sampled_real_info.json'
        gen_info_path = '/dexycb/dexycb_s0/similarity_sampled_gen_info.json'
    elif args.data_split == 's1_train':
        real_info_path = '/dexycb/dexycb_s1/sampled_real_info.json'
        gen_info_path = '/dexycb/dexycb_s1/similarity_sampled_gen_info.json'
    else:
        raise RuntimeError
    ...
    for idx, (ann, img) in tqdm(enumerate(zip(anns, imgs)), total=num_data, desc=f'current split: {args.current_split}'):
        ...
        img_path = os.path.join(args.root_dir, img['file_name'])
        ...
        # generated grasping
        # NOTE: v3
        if (random_number < 0.8 or object_code_id == 19) and (object_code_id != 18):
            ...
            # random choose a root_joint_coord_cam, mano_shape, global_rotation
            root_joint_coord_cam_files = glob.glob('dexycb/root_joint_coord_cam/*.json')
        ...
        elif (random_number >= 0.8 or object_code_id == 18) and (object_code_id != 19):
            # random choose a mano_pose, object_pose
            ...
            img_fn = data['img_fn']
            img_path = os.path.join('/data/data/DexYCB', img_fn)
            cam_param = data['cam_param']
            hand_type = data['hand_type']  # right: 1, left: 0
            do_flip = not bool(hand_type)
            assert not do_flip
            ...
            # current_pose_y[:3, :3] = np.dot(rot_aug_mat, current_pose_y[:3, :3])
            current_pose_y[:3, 3] -= joints_coord_cam[0]
            current_pose_y = np.dot(rot_aug_mat, current_pose_y)
            current_pose_y[:3, 3] += joints_coord_cam[0]
            pose_y[meta['ycb_grasp_ind']] = current_pose_y
        ...
        # if True:  # for debug
        if idx % 200 == 0:
            save_dir = f'grasp_{args.data_split}_{args.suffix}_v{args.version}_p{args.part}'
            os.makedirs(f'./debug/{save_dir}', exist_ok=True)
            # image
            color = load_img(img_path)  # TODO original image is replaced by other generated imgs by accident
            if color.shape[0] != normal.shape[0]:
                # print('wrong original image, return black img!')  # for debug
...
if __name__ == '__main__':
    ...
    parser.add_argument('--root_dir', '-r', type=str, default='/data/data/DexYCB', help='root data dir')
