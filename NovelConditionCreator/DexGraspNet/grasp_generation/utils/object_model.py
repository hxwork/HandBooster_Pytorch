...
class ObjectModel:
    ...
    def __init__(self, data_root_path, batch_size_each, num_samples=2000, device="cuda", random_scale=True):
        """
    ...
    def initialize(self, object_code_list):
        ...
        for object_code in object_code_list:
            ...
            self.object_mesh_list.append(tm.load(os.path.join(self.data_root_path, object_code, "textured_simple.obj"), force="mesh", process=False))
        ...
        if self.num_samples != 0:
            self.surface_points_tensor = torch.stack(self.surface_points_tensor, dim=0).repeat_interleave(self.batch_size_each,
                                                                                                          dim=0)  # (n_objects * batch_size_each, num_samples, 3)
