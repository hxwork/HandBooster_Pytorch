...
def generate_object_pose(_):
    ...
    if os.path.exists(os.path.join(args.data_root_path, object_code, 'coacd',
                                   'coacd.urdf')) and not os.path.exists(os.path.join(args.data_root_path, object_code, 'coacd', 'coacd_convex_piece_63.obj')):
        ...
        builder.add_collision_from_file(os.path.join(args.data_root_path, object_code, 'textured_simple.obj'))
    ...
    for i in range(args.n_samples):
        ...
        start_pose_matrices.append(object_actor.get_pose().to_transformation_matrix())
        # simulate
        for t in range(args.sim_steps):
            scene.step()
        pose_matrices.append(object_actor.get_pose().to_transformation_matrix())
    start_pose_matrices = np.stack(start_pose_matrices)
    pose_matrices = np.stack(pose_matrices)

    # save results
    np.save(os.path.join(args.poses, object_code + '_start.npy'), start_pose_matrices)
    ...
    if os.path.exists(os.path.join(args.data_root_path, object_code, 'textured_simple.obj.convex.stl')):
        os.remove(os.path.join(args.data_root_path, object_code, 'textured_simple.obj.convex.stl'))
