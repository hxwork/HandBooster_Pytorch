...
class hand_regHead(nn.Module):
    ...
    def __init__(self, roi_res=32, joint_nb=21, stacks=1, channels=256, blocks=1):
        ...
        center_offset = 0.5
        vv, uu = torch.meshgrid(torch.arange(self.out_res).float(), torch.arange(self.out_res).float(), indexing="ij")
    ...
    def make_residual(self, block, inplanes, planes, blocks, stride=1):
        ...
        if stride != 1 or inplanes != planes * block.expansion:
            skip = nn.Sequential(nn.Conv2d(inplanes, planes * block.expansion, kernel_size=1, stride=stride, bias=True))
        layers = []
    ...
    def generate_output(self, heatmaps):
        predictions = torch.stack(
            (torch.sum(torch.sum(heatmaps * self.uu, dim=2), dim=2), torch.sum(torch.sum(heatmaps * self.vv, dim=2), dim=2)), dim=2)
        return predictions
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, in_planes, out_planes, kernel_size, groups=1):
        ...
        self.block = nn.Sequential(
            nn.Conv2d(in_planes, out_planes, kernel_size=kernel_size, stride=1, padding=((kernel_size - 1) // 2), groups=groups, bias=True),
            nn.BatchNorm2d(out_planes), nn.LeakyReLU(inplace=True))
...
class hand_Encoder(nn.Module):
    ...
    def __init__(self, num_heatmap_chan=21, num_feat_chan=256, size_input_feature=(32, 32), nRegBlock=4, nRegModules=2):
