...
from model.mob_recon.models.transformer import Transformer, SelfAttentionLayer
...
def init_weights(m):
    ...
    # elif type(m) == nn.Conv2d or type(m) == nn.Conv1d:
    #     nn.init.normal_(m.weight, std=0.001)
        ...
        for idx in range(len(self.out_channels)):
            if idx == 0:
                self.de_layer.append(
                    SpiralDeblock(self.out_channels[-idx - 1],
                                  self.out_channels[-idx - 1],
                                  self.spiral_indices[-idx - 1],
                                  meshconv=meshconv))
            else:
                self.de_layer.append(
                    SpiralDeblock(self.out_channels[-idx], self.out_channels[-idx - 1], self.spiral_indices[-idx - 1], meshconv=meshconv))
