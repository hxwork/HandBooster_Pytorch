...
from model.mob_recon.models.loss import l1_loss, normal_loss, edge_length_loss, contrastive_loss_3d, contrastive_loss_2d
...
#         self.cfg = cfg
#         self.backbone = DenseStack_Backnone(latent_size=cfg.MODEL.LATENT_SIZE, kpts_num=cfg.MODEL.KPTS_NUM)
#         cur_dir = os.path.dirname(os.path.realpath(__file__))
#         template_fp = os.path.join(cur_dir, '../../template/template.ply')
#         transform_fp = os.path.join(cur_dir, '../../template', 'transform.pkl')
#         spiral_indices, _, up_transform, tmp = spiral_tramsform(transform_fp, template_fp, cfg.MODEL.SPIRAL.DOWN_SCALE,
#                                                                 cfg.MODEL.SPIRAL.LEN, cfg.MODEL.SPIRAL.DILATION)
...
#         return {'verts': pred3d, 'joint_img': pred2d_pt}
...
#         else:
#             loss_dict['normal_loss'] = 0.1 * normal_loss(kwargs['verts_pred'], kwargs['verts_gt'], kwargs['face'].to(
#                 kwargs['verts_pred'].device))
#             loss_dict['edge_loss'] = edge_length_loss(kwargs['verts_pred'], kwargs['verts_gt'],
#                                                       kwargs['face'].to(kwargs['verts_pred'].device))
