...
def augmentation_gen(img, bbox, input_img_shape, scale_factor=0.25, rot_factor=30, rot_prob=0.6, color_factor=0.2):

    scale = np.clip(np.random.randn(), -1.0, 0.) * scale_factor + 1.0
    rot = np.clip(np.random.randn(), -2.0, 2.0) * rot_factor if random.random() <= rot_prob else 0
...
def generate_patch_image(cvimg, bbox, scale, rot, do_flip, out_shape, warp_flag=cv2.INTER_LINEAR):
    img = cvimg.copy()
    ...
    img_patch = cv2.warpAffine(img, trans, (int(out_shape[1]), int(out_shape[0])), flags=warp_flag)
...
def generate_patch_trans(bbox, scale, rot, do_flip, in_shape, out_shape):
    img_height, img_width = in_shape
    ...
    return trans, inv_trans, rot, scale
